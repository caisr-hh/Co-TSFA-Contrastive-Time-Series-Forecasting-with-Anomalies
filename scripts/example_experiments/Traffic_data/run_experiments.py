...
# hyper-params (preserved exactly)
...
# base TimesNet config (exactly as provided)
BASE_CMD = [
    "python",
    "-u", "./run.py",
...
def build_cmd(
    seed, train_cat, test_cat,
    continuous_ano_type=None, pointwise_ano_type=None, pointwise_ratio=None
):
    cmd = BASE_CMD + [
        "--seed", str(seed),
    ...
    if (train_cat == "continuous") or (test_cat == "continuous"):
        ctype = continuous_ano_type or continuous_ano_types[0]
        cmd += ["--continuous_ano_type", ctype]
...
for train_cat, test_cat in product(train_categories, test_categories):
    ...
    sweep_cont = (train_cat == "continuous") or (test_cat == "continuous")
    sweep_point = (train_cat == "pointwise") or (test_cat == "pointwise")

    if sweep_cont and sweep_point:
        param_grid = list(product(
            continuous_ano_types, pointwise_ano_types, pointwise_ratios
        ))
    elif sweep_cont:
        param_grid = [(ctype, None, None) for ctype in continuous_ano_types]
    elif sweep_point:
        param_grid = [(None, ptype, pr) for ptype in pointwise_ano_types for pr in pointwise_ratios]
    else:
    ...
    for cont_type, ptype, pratio in param_grid:
        ...
        for seed in seeds:
            ...
            if log_tag in done_tags:
                print(f"Skipping completed: {log_tag}")
                continue
            ...
            if mae is None or mse is None:
                print(f"Could not parse metrics for {log_tag}")
                continue
