    ...
    for ano_type in ano_types:
        for method in methods:
            ...
            for cl_weight in weights:
                ...
                log_file = (
                    f"logs/timexer_{method}_{ano_type}_clw{cl_weight}_seed{seed}.log"
                )
                ...
                print(
                    f"Running TimeXer ETTh1: seed={seed}, "
                    f"cl_weight={cl_weight}, ano_type={ano_type}, method={method}"
                )

                # Execute experiment and save log
                with open(log_file, "w") as lf:
                    subprocess.run(cmd, stdout=lf, stderr=lf, text=True)

                # Extract and append RESULT lines
                subprocess.run(
