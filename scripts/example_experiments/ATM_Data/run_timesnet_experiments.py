...
# Ensure logs and results folder exist
...
for seed in seeds:
    for ano_type in ano_types:
        for method in methods:
            ...
            # Only apply the actual CL weight when supervised_cl
            weights = cl_weights if method == "Co_TSFA" else [0]

            for cl_weight in weights:

                # Log path
                log_file = f"logs/TimesNet_{method}_{ano_type}_clw{cl_weight}_seed{seed}.log"

                # Build the command as a list (avoids shell quoting issues)
                cmd = [
                    "python",
                    "-u",
                    "./run.py",
                ...
                print(
                    f"Running: seed={seed}, cl_weight={cl_weight}, "
