...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        with torch.no_grad():
            for i, batch in enumerate(vali_loader):
                if len(batch) == 7:
                    batch_x, batch_y, batch_x_mark, batch_y_mark, _, _, _ = batch
                else:
                    # ETT: no augmented/clean or mask
    ...
    def train(self, setting):
        print("test on anomalies flag: ", self.args.test_on_anomalies)
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, batch in enumerate(train_loader):
                ...
                # -------------------- CL ADDITION START ------------------------
                if epoch > 0:  # <-- skip CL in the first epoch
                    ...
                    en_embed, _ = self.model.en_embedding(batch_x.permute(0, 2, 1))
                    ex_embed = self.model.ex_embedding(batch_x, batch_x_mark)
                    enc_out_clean = self.model.encoder(en_embed, ex_embed)   # [B, T, D]
                    ...
                    for _ in range(5):
                        ...
                        en_embed_anom, _ = self.model.en_embedding(batch_x_anom.permute(0, 2, 1))
                        ex_embed_anom = self.model.ex_embedding(batch_x_anom, batch_x_mark)
                        enc_out_anom = self.model.encoder(en_embed_anom, ex_embed_anom)
                    ...
                    # Only forecasting in the 1st epoch
                    loss = forecast_loss
                # ----------------------- CL ADDITION END --------------------------
    ...
    def _evaluate_loader(self, loader, test_data, setting, tag="normal"):
        ...
        with torch.no_grad():
            for i, batch in enumerate(loader):
                if len(batch) == 7:
                    batch_x, batch_y, batch_x_mark, batch_y_mark, atm_ids, dates_x, dates_y = batch
                else:
                    # ETT: no augmented/clean or mask
                    batch_x, batch_y, batch_x_mark, batch_y_mark = batch
                    atm_ids, dates_x, dates_y = None, None, None
        ...
        # SMAPE on inverse
        inv_preds, inv_trues, inv_inputs = preds.copy(), trues.copy(), inputs.copy()

        if test_data.scale:
            shape_pred = inv_preds.shape
            inv_preds = test_data.inverse_transform(inv_preds.reshape(shape_pred[0] * shape_pred[1], -1)).reshape(shape_pred)

            shape_true = inv_trues.shape
            inv_trues = test_data.inverse_transform(inv_trues.reshape(shape_true[0] * shape_true[1], -1)).reshape(shape_true)

            shape_inp = inv_inputs.shape
            inv_inputs = test_data.inverse_transform(inv_inputs.reshape(shape_inp[0] * shape_inp[1], -1)).reshape(shape_inp)

        smape_val = smape(inv_trues, inv_preds)
    ...
    def test(self, setting, test=0):
        ...
        if self.args.test_on_anomalies == 1:
            test_data_anom, test_loader_anom = self._get_data(flag='test_anom')
