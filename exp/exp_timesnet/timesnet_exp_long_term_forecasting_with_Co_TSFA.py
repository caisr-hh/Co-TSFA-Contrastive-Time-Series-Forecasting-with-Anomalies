...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        self.model.eval()
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, _, _, _) in enumerate(vali_loader):
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, _, _, _) in enumerate(train_loader):
                iter_count += 1
                ...
                if self.args.use_amp:
                    ...
                    aug_true_list = []
                    #aug_pred_list = [] uncomment this if we want to use predictions instead
                    z_aug_list = []
                    
                    for _ in range(5):
                        #this if statement should be like this since model should learn differences between normal sequences and anomaloues sequences based on what anomalies there are in the test data, not the training data. feels weird to have test there so changed to train
    ...
    def _evaluate_loader(self, loader, test_data, setting, tag="normal"):
        ...
        self.model.eval()
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, atm_ids, dates_x, dates_y) in enumerate(loader): #added dates_x and dates_y
                batch_x = batch_x.float().to(self.device)
                batch_y = batch_y.float().to(self.device)
                ...
                preds.append(pred)
                trues.append(true)
                atm_id_list.extend(atm_ids) #added
