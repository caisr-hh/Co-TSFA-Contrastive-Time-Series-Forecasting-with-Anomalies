            ...
            for i, batch in enumerate(train_loader):
                ...
                if self.args.use_amp:
                    ...
                    def pattn_encode(x):
                        ...
                        B, _, C = x.shape
                        x = x.permute(0, 2, 1)
                        x = self.model.padding_patch_layer(x)
                        x = x.unfold(dimension=-1, size=self.model.patch_size, step=self.model.stride)

                        # Linear projection + encoder
                        enc = self.model.in_layer(x)
                        enc = rearrange(enc, 'b c m d -> (b c) m d')
                        z, _ = self.model.encoder(enc)
                        return z

                    # Clean latent (keep gradient)
                    enc_out_clean = pattn_encode(batch_x)
                    ...
                    for j in range(5):
                        ...
                        aug_x_list.append(seq_x_anom_np.copy())
                        aug_y_list.append(seq_y_anom_np.copy())
                        
                        # Convert anomalous batch
                        ...
                        # -------- ANOMALOUS LATENT (PATTn PIPELINE) --------
                        enc_out_anom = pattn_encode(batch_x_anom)

                        z_aug_list.append(enc_out_anom)

                    # Contrastive loss
                    ida_cl = Co_TSFA_contrastive_loss(
                        orig_clean=batch_y,
                        orig_aug_list=aug_true_list,
                        z_clean=enc_out_clean,   # gradients flow here
                        z_aug_list=z_aug_list    # no gradients here
                    )
