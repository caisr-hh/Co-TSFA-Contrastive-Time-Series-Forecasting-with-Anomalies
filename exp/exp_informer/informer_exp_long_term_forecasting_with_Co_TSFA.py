...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def train(self, setting):
        ...
        # Store batch-level loss every 10 steps
        self.batch_loss_log = []       # total loss
        self.batch_forecast_log = []   # only forecast loss
        self.batch_cl_log = []         # only contrastive loss
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, batch in enumerate(train_loader):
                ...
                if self.args.use_amp:
                    ...
                    # Clean latent representation (encoder only)
                    enc_out = self.model.enc_embedding(batch_x, batch_x_mark)
                    enc_out_clean, _ = self.model.encoder(enc_out, attn_mask=None)

                    aug_true_list = []
                    z_aug_list = []
                    
                    random_num = random.randint(0, self.args.batch_size - 1)
                    plots_data = []

                    for j in range(5):
                        # inject anomalies (same code you had before)
                        ...
                        # Latent representation for anomalous sequence
                        enc_out_anom = self.model.enc_embedding(batch_x_anom, batch_x_mark)
                        enc_out_anom, _ = self.model.encoder(enc_out_anom, attn_mask=None)

                        z_aug_list.append(enc_out_anom)

                    # Contrastive loss on encoder outputs
